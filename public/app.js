...
const percentileLabel = (rank) => {
  if (!Number.isFinite(Number(rank)) || Number(rank) <= 0) return "UNRANKED";
  const value = state.walletCount ? Number(rank) / state.walletCount * 100 : 0;
  return `TOP ${value < 0.01 ? "<0.01" : value.toFixed(2)}%`;
};
const rankLabel = (rank) => Number.isFinite(Number(rank)) && Number(rank) > 0 ? `#${fmt(rank)}` : "UNRANKED";
...
function renderPendleAnalytics(data) {
  ...
  return `<section class="pendle-analytics"><h3>PENDLE YT ANALYTICS <span>${positions.length} ${positions.length === 1 ? "MARKET" : "MARKETS"} · OFFICIAL PENDLE DATA</span></h3><div class="pendle-list">${positions.map((position, index) => {
    ...
    return `<article><div class="pendle-title"><div><span>YT-${escapeHtml(position.name)}</span><small>MATURITY ${escapeHtml(expiry)} · CHAIN ${fmt(position.chainId)}</small></div><div class="pendle-title-actions"><strong>${position.matured ? "MATURED" : position.closed ? "CLOSED" : fmt(position.balance)} <small>${position.matured ? `${fmt(position.balance || 0)} YT AT MATURITY` : position.closed ? `${fmt(position.soldUnits || 0)} YT SOLD` : `YT · ${currency.format(position.currentYtValueUsd || 0)}`}</small></strong><button type="button" class="pendle-share-button" data-pendle-index="${index}">Share YT PnL <b>↗</b></button></div></div><dl><div><dt>${position.closed ? "PEAK CAPITAL DEPLOYED" : "CURRENT COST BASIS"}</dt><dd>${currency.format(position.entryCostUsd)}</dd><small>${position.closed ? `BASE ${currency.format(position.baseBagUsd || 0)} · REINVESTED ${currency.format(position.reinvestedBagUsd || 0)}` : `LIFETIME PEAK ${currency.format(position.peakCapitalUsd || position.entryCostUsd)}`}</small></div><div><dt>AVG. DEPLOYED / YT</dt><dd>${escapeHtml(entry)} <small>ASSET</small></dd></div><div><dt>YIELD CLAIMED</dt><dd class="positive">${currency.format(position.claimedYieldUsd)}</dd><small>${fmt(position.yieldClaims)} CLAIMS · ${fmt(position.claimedYieldAsset)} ASSET</small></div>${yieldOrExit}<div><dt>YT TOTAL PNL</dt><dd class="${position.ytTotalPnlUsd >= 0 ? "positive" : "negative"}">${currency.format(position.ytTotalPnlUsd)}</dd><small>${position.matured ? "FINAL PENDLE NET GAIN AT MATURITY" : position.closed ? "REALIZED AFTER EARLY EXIT + YIELD" : "LIFETIME PNL · CURRENT BAG MARKED TO MARKET"}</small></div><div><dt>UNDERLYING / IMPLIED</dt><dd>${position.underlyingApy == null ? "—" : percent.format(position.underlyingApy)} <small>/ ${position.impliedApy == null ? "—" : percent.format(position.impliedApy)}</small></dd></div></dl></article>`;
...
function openPendleSocialCard(position, wallet) {
  const returnCapital = Number(position.peakCapitalUsd || position.entryCostUsd);
  const pnlReturn = returnCapital ? Number(position.ytTotalPnlUsd) / returnCapital : 0;
